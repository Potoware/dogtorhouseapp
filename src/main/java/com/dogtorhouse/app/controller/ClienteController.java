...
public class ClienteController extends BaseController {
	...
	public String guardarCliente(@Valid Cliente cliente, BindingResult result, Model model, SessionStatus status,
		...
		if ((cliente.getId() == null
				|| !clienteService.findById(cliente.getId()).get().getEmail().equals(cliente.getEmail()))
				&& !clienteService.validoEmail(cliente.getEmail().trim().toLowerCase())) {
	...
	public String editarClienteForm(@PathVariable(value = "idCliente") Long id, Map<String, Object> model,
			RedirectAttributes redirectAttributes, HttpSession session) {
		if (!usuarioLogueado((Veterinario) session.getAttribute("veterinarioSesion"))) {
			return "redirect:/login";
		}
