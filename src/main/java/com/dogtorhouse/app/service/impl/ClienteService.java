...
public class ClienteService implements IClienteService{
	...
	public List<Cliente> findAll() {
		return ((List<Cliente>) clienteRepository.findAll()).stream()
				.filter(cliente->Objects.isNull(cliente.getFechaBaja()))
	...
	public List<Cliente> findAllCriterioCliente(CriterioCliente criterio) {
	    return ((List<Cliente>)clienteRepository.findAll()).stream()
	        .filter(cliente -> Utilidades.isBlankOrNull(criterio.getIdentificacion()) || cliente.getIdentificacion().contains(criterio.getIdentificacion()))
	        .filter(cliente -> Utilidades.isBlankOrNull(criterio.getNombres()) || cliente.getNombres().toLowerCase().contains(criterio.getNombres().toLowerCase()))
	        .filter(cliente -> Utilidades.isBlankOrNull(criterio.getApellidos()) || cliente.getApellidos().toLowerCase().contains(criterio.getApellidos().toLowerCase()))
	        .filter(cliente -> Utilidades.isBlankOrNull(criterio.getEmail()) || cliente.getEmail().toLowerCase().contains(criterio.getEmail().toLowerCase()))
	        .filter(cliente -> Objects.isNull(cliente.getFechaBaja()))
	        .collect(Collectors.toList());
	}
