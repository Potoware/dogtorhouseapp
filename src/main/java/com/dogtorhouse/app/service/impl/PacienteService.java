...
public class PacienteService implements IPacienteService {
	...
	public List<Paciente> findAll() {
		...
		return ((List<Paciente>) pacienteRepository.findAll()).stream()
				.filter(paciente -> Objects.isNull(paciente.getFechaBaja())).collect(Collectors.toList());
	}
	...
	public List<Paciente> findAllCriterioPaciente(CriterioPaciente criterio) {
		...
		return ((List<Paciente>) pacienteRepository.findAll()).stream()
				...
				.filter(paciente -> Utilidades.isBlankOrNull(criterio.getEspecie())
						|| paciente.getEspecie().toLowerCase().contains(criterio.getEspecie().toLowerCase()))
				.filter(paciente -> Utilidades.isBlankOrNull(criterio.getNombre())
						|| paciente.getNombre().toLowerCase().contains(criterio.getNombre().toLowerCase()))
				.filter(paciente -> Utilidades.isBlankOrNull(criterio.getRaza())
						|| paciente.getRaza().toLowerCase().contains(criterio.getRaza().toLowerCase()))
				.filter(paciente -> criterio.getCliente() == null
						|| paciente.getCliente().getId().equals(criterio.getCliente()))
				.filter(paciente -> Objects.isNull(paciente.getFechaBaja())).collect(Collectors.toList());
