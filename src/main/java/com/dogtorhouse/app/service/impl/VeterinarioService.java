...
public class VeterinarioService implements IVeterinarioService {
	...
	public List<Veterinario> findAll() {
		...
		return ((List<Veterinario>) veterinarioRepository.findAll()).stream()
				.filter(veterinario -> Objects.isNull(veterinario.getFechaBaja())).collect(Collectors.toList());
	}
	...
	public List<Veterinario> findAllCriterioVeterinario(CriterioVeterinario criterio) {
		...
		return ((List<Veterinario>) veterinarioRepository.findAll()).stream()
				.filter(veterinario -> Utilidades.isBlankOrNull(criterio.getIdentificacion())
						|| veterinario.getIdentificacion().contains(criterio.getIdentificacion()))
				// .filter(veterinario ->criterio.getActivo() ||
				// veterinario.getActivo().equals(criterio.getActivo()))
				.filter(veterinario -> Utilidades.isBlankOrNull(criterio.getNombres())
						|| veterinario.getNombres().toLowerCase().contains(criterio.getNombres().toLowerCase()))
				.filter(veterinario -> Utilidades.isBlankOrNull(criterio.getApellidos())
						|| veterinario.getApellidos().toLowerCase().contains(criterio.getApellidos().toLowerCase()))
				.filter(veterinario -> Utilidades.isBlankOrNull(criterio.getEmail())
						|| veterinario.getEmail().toLowerCase().contains(criterio.getEmail().toLowerCase()))
				.filter(veterinario -> Objects.isNull(veterinario.getFechaBaja())).collect(Collectors.toList());
